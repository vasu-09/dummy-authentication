...
public class OtpServiceTest {
    ...
    void verifyOtp_redisHit_deletesAndReturnsUserId() {
        ...
        when(userRepo.findByPhone(e164)).thenReturn(Optional.of(user));

        Long id = service.verifyOtp(phone, "123456");
        assertEquals(42L, id);
        verify(redis).delete(key);
        verify(otpRepo, never()).findByPhone(any());
    }
    ...
    void verifyOtp_dbFallback_whenEnabled() {
        ...
        when(otpRepo.findByPhone(e164)).thenReturn(Optional.of(row));
        User user = new User();
        user.setId(7L);
        user.setPhoneNumber(e164);
        when(userRepo.findByPhone(e164)).thenReturn(Optional.of(user));
